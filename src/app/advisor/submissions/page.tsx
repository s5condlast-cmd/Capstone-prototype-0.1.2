...
export default function AdvisorSubmissions() {
  ...
  const handleAction = (id: string, action: "approved" | "rejected" | "revision") => {
    const updated = submissions.map(s => s.id === id ? { ...s, status: action, feedback: feedback || s.feedback } as Submission : s);
        ...
        <div className="flex-1 w-full space-y-6">
          ...
          <div className="flex items-center gap-2 bg-white dark:bg-slate-900 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm w-fit">
            {["all", "pending", "approved", "rejected", "revision"].map(f => (
              <button key={f} onClick={() => setFilter(f)} className={`px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors ${filter === f ? "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400" : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>{f}</button>
            ))}
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            {filtered.length === 0 ? (
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {filtered.map(sub => (
                  <div key={sub.id} onClick={() => setSelectedSubmission(sub)} className={`px-6 py-4 flex items-center justify-between cursor-pointer transition-colors ${selectedSubmission?.id === sub.id ? "bg-blue-50 dark:bg-blue-900/10" : "hover:bg-slate-50 dark:hover:bg-slate-800/30"}`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="text-sm font-semibold text-slate-800 dark:text-slate-200">{sub.title || "Untitled"}</div>
                        <div className="text-xs text-slate-500">{sub.studentName || sub.studentId || "Unknown"} · {(sub.type || "doc").toUpperCase()} · {new Date(sub.submittedAt).toLocaleDateString()}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {sub.isUrgent && <span className="bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-[10px] font-bold px-2 py-0.5 rounded-full">URGENT</span>}
          ...
          {selectedSubmission ? (
            <>
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 mb-4">Submission Details</h3>
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800/50">
                    <span className="text-slate-500">Student</span>
                    <span className="font-medium text-slate-800 dark:text-slate-200">{selectedSubmission.studentName || selectedSubmission.studentId || "Unknown"}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800/50">
                ...
                {selectedSubmission.content && (
                  <div className="mt-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50">
                    <p className="text-xs font-bold text-slate-500 mb-2">CONTENT</p>
                ...
                {selectedSubmission.feedback && (
                  <div className="mt-4 p-4 rounded-xl bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30">
                    <p className="text-xs font-bold text-amber-600 dark:text-amber-400 mb-2">PREVIOUS FEEDBACK</p>
              ...
              {/* Actions */}
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 mb-4">Review Actions</h3>
                <textarea
                  value={feedback}
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={() => handleAction(selectedSubmission.id, "approved")} className="w-full py-2.5 rounded-xl text-sm font-semibold bg-green-600 hover:bg-green-700 text-white transition-colors">✓ Approve</button>
                  <button onClick={() => handleAction(selectedSubmission.id, "revision")} className="w-full py-2.5 rounded-xl text-sm font-semibold bg-amber-500 hover:bg-amber-600 text-white transition-colors">↺ Request Revision</button>
                  <button onClick={() => handleAction(selectedSubmission.id, "rejected")} className="w-full py-2.5 rounded-xl text-sm font-semibold bg-red-600 hover:bg-red-700 text-white transition-colors">✕ Reject</button>
                </div>
              </div>
            </>
          ) : (
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center py-12">
              ...
              <p className="text-xs text-slate-500 mt-1">Click on any submission to view details and take action.</p>
            </div>
          )}
        </div>
      </div>
