...
export default function AdminAdviserAssignment() {
  ...
  const handleAssign = (studentId: string) => {
    ...
    if (!confirm("Save assignment?")) return;
                    ...
                    <tr key={student.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-3 px-6">
                        <div className="flex items-center gap-2">
                          <select 
                            value={pendingAssignments[student.studentId] !== undefined ? pendingAssignments[student.studentId] : (assignment?.adviserId || "")} 
                            onChange={e => setPendingAssignments(prev => ({ ...prev, [student.studentId]: e.target.value }))} 
                            className="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-slate-100 text-sm outline-none"
                          >
                            <option value="">Select Adviser</option>
                            {advisors.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                          </select>
                          {pendingAssignments[student.studentId] !== undefined && pendingAssignments[student.studentId] !== (assignment?.adviserId || "") && (
                            <button 
