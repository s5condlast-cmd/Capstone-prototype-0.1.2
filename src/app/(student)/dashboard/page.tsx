  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <p className="text-[13px] text-[hsl(var(--muted-foreground))]">Hello, <span className="text-clickable font-semibold">Student User</span></p>
          <h1 className="text-[28px] font-bold tracking-tight mt-1">Student Dashboard</h1>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))]">
          <div className="flex flex-col items-end leading-tight">
            <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[hsl(var(--muted-foreground))]">Global Status</span>
            <span className="text-[12px] font-semibold uppercase text-sti-blue mt-1">In Progress</span>
          </div>
      ...
      <Card className="overflow-hidden border border-[hsl(var(--border))] shadow-sm bg-[hsl(var(--card))]">
        <CardContent className="p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[hsl(var(--muted))] text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--foreground))]">
              Overall Completion
            </div>
            <h3 className="text-[26px] md:text-[34px] font-bold tracking-tight">You’ve completed {completionPct}% of your required documents</h3>
            <p className="text-[13px] text-[hsl(var(--muted-foreground))]">You have {REQUIRED_TYPES.length - submittedTypes.length} requirement type(s) left to submit.</p>
          </div>
          <div className="text-right">
            <div className="text-[44px] font-bold tracking-tight tabular-nums leading-none">{completionPct}%</div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[hsl(var(--muted-foreground))] mt-2">Finalized</p>
          </div>
        </CardContent>
      </Card>
        ...
        <Card className="border border-[hsl(var(--border))] shadow-sm bg-[hsl(var(--card))] overflow-hidden">
          <CardHeader className="border-b border-[hsl(var(--border))]">
            <div className="flex items-center justify-between gap-4">
              <div>
                <CardTitle className="text-[15px]">Submission Timeline</CardTitle>
                <CardDescription className="text-[13px]">Track recent uploads and status updates.</CardDescription>
              </div>
              <Link href="/documents" className="text-clickable text-[12px] font-semibold">
                Manage files →
              </Link>
            </div>
          </CardHeader>
          <CardContent className="p-0">
            {submissions.length === 0 ? (
              <div className="py-20 text-center text-[13px] text-[hsl(var(--muted-foreground))]">No submissions yet. Start with the quick actions.</div>
            ) : (
                  ...
                  <div key={i} className="flex items-center justify-between gap-4 px-5 py-4 hover:bg-[hsl(var(--muted))]/50 transition-colors group">
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-[13px] font-semibold truncate">{sub.title}</p>
                        <p className="text-[11px] text-[hsl(var(--muted-foreground))] uppercase tracking-wider mt-0.5">{sub.type} • {sub.date || "No date"}</p>
                      </div>
                    </div>
                    <Badge variant="outline" className="text-[10px] font-semibold uppercase tracking-wider h-5 px-2 shrink-0">
                      {sub.status}
                    </Badge>
                  </div>
                ))}
        ...
        <div className="flex flex-col gap-6">
          <Card className="border border-[hsl(var(--border))] shadow-sm bg-[hsl(var(--card))]">
            <CardHeader>
              ...
              <CardDescription className="text-[13px]">Jump straight to the next task.</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-2">
              {[
                ...
                <Link key={item.name} href={item.href} className="flex items-center justify-between rounded-lg border border-transparent px-3 py-3 hover:border-[hsl(var(--border))] hover:bg-[hsl(var(--muted))] transition-all group">
                  <span className="text-clickable text-[13px] font-semibold">{item.name}</span>
          ...
          <div className="rounded-xl bg-[hsl(var(--muted))] border border-[hsl(var(--border))] p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-[12px] font-semibold uppercase tracking-[0.18em] text-[hsl(var(--muted-foreground))]">Need a Template?</h4>
              <LayoutGrid className="h-4 w-4 text-[hsl(var(--muted-foreground))]" />
            </div>
            <p className="text-[12px] text-[hsl(var(--muted-foreground))] leading-relaxed">Browse official forms and downloadable templates for your practicum requirements.</p>
            <Link href="/templates" className="block text-center h-9 leading-9 rounded-lg bg-[hsl(var(--foreground))] text-[hsl(var(--background))] text-[12px] font-semibold hover:opacity-90 transition-opacity">
              Open Templates
            </Link>
          </div>
        </div>
