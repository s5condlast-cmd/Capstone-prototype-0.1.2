# Design Information & Inspiration

## 🎯 Professional Reference (Inspiration)

To make the system look highly professional—especially for data backup and system storyboarding (audit trails/timeline of events)—we should draw inspiration from standard enterprise SaaS dashboards like **Vercel's Project Settings**, **Supabase's Dashboard**, or **Stripe's Developer Settings**. 

### Why these platforms?
1. **Clean Card-Based Layouts:** They use thin borders (`border-slate-200`), soft shadows (`shadow-sm`), and clear typography. This perfectly matches the Tailwind styling we already established in your `AdminLayout`.
2. **Danger Zones vs. Safe Zones:** They clearly separate "safe" actions (like Exporting Data) from "destructive" actions (like Wiping the Database).
3. **Event Storyboarding:** They feature elegant vertical timelines (Audit Logs) that tell the "story" of the system—showing exactly who did what and when.

---

## 🛠 Proposed Page: `Data & Storyboard` (`/admin/data-storyboard`)

This page serves as the master control room to **"back everything up"** and view the **"storyboard"** (the chronological audit trail of the system).

### Core Components to Include:

1. **System Backup Engine (Back everything up)**
   - **Export Full System Data:** Buttons to download Users, Submissions, and Batches as CSV or JSON.
   - **Archive Semester:** A button to package all approved documents into a ZIP file at the end of the academic year.
   - **Cloud Sync:** A button (UI placeholder) to sync data to external storage.

2. **Activity Storyboard (Audit Trail)**
   - A vertical timeline showing the narrative of the app.
   - E.g., *"Admin updated access control roles - 2 mins ago"*
   - E.g., *"Student A submitted Final Evaluation - 1 hour ago"*
   - Includes filtering buttons (Filter by User, Filter by Action).

---

## 💻 Ready-to-Copy Implementation Code

You can create a new page at `src/app/admin/(system)/data-storyboard/page.tsx` and paste the following code. It maintains your UI standards and looks incredibly professional:
          ...
          <div className="lg:col-span-2">
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm h-full">
              ...
              <div className="relative border-l-2 border-slate-100 dark:border-slate-800 ml-4 space-y-8">
                {storyboardEvents.map((event, index) => (
